...
let
    ...
    if isnothing(initstate_file)
        sites = siteinds(n -> n == 1 ? "vFDot3" : "vFermion", total_size)
        initialsites = Dict(
            [
                dot_site => "Vacuum"
    ...
    L = MPO(
        dot_hamiltonian(SiteType("vFDot3"), dot_energies, dot_coulomb_repulsion, dot_site) +
        empty_chain_coups[1] *
        exchange_interaction(SiteType("vFDot3"), dot_site, empty_chain_range[1]) +
        filled_chain_coups[1] *
        exchange_interaction(SiteType("vFDot3"), dot_site, filled_chain_range[1]) +
        spin_chain(
