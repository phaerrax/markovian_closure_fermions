...
let
    ...
    #targetop = parameters["target_operator"]
    #targetop = MPS(sites, [i == targetop.pos ? targetop.op : "vecId" for i in 1:length(sites)])
    targetop0 = MPS(sites, ["vN"; repeat(["vId"], length(sites) - 1)])
    targetop, _ = stretchBondDim(targetop0, parameters["max_bond"])
    opgrade = "even"

    # Instead of calculating the evolution of the state, we calculate the evolution, in the
    # Heisenberg picture, directly of the operator we want to measure the expectation value
    # of. This should result in a lighter simulation, resource-wise, but of course we will
    # be able to simulate only one operator at a time.
    # The operator is an MPS representing its vectorization, just like any state. 
    # Another advantage is that (following Arrigoni et al.) the equation of motion for
    # operators is always valid, i.e. we need fewer assumptions on the initial state.
    adjℓ = OpSum()
