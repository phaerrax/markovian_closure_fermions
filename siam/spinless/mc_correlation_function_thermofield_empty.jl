...
let
    ...
    # Site ranges
    empty_chain_range = range(; start=1, step=2, length=chain_length)
    empty_closure_range = range(;
        start=empty_chain_range[end] + 2, step=2, length=closure_length
    )
    filled_chain_range = range(; start=2, step=2, length=chain_length)
    ...
    # This is the only relevant correlation function of the empty branch of the environment
    ...
    ρ₀c₁ = empty_chain_coups[1] * MPS(ComplexF64, sites, [initialsites[i] for i in 1:total_size])
    ρ₀c₁[empty_chain_range[1]] = noprime(
        op("⋅σ-", sites, empty_chain_range[1]) * ρ₀c₁[empty_chain_range[1]]
    )
    ...
    initialops = Dict(
        [
            ...
            [st => "vC" for st in empty_chain_range[1]]
            [st => "vId" for st in empty_chain_range[2:end]]
            [st => "vId" for st in empty_closure_range]
        ],
    )
    c₁ = empty_chain_coups[1] * MPS(ComplexF64, sites, [initialops[i] for i in 1:total_size])
